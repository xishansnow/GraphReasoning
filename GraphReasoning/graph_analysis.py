...
def find_path_and_reason(
    ...
    tokenizer,
    model,
    generate,  # a function that generates text given a prompt
    keyword_1="music and sound",
    # local_llm=None,
    keyword_2="apples",
    ...
    """Find a path between two keywords in a graph, extract information based on the specified analysis type, and generate a reasoned response.
    ️Args:
    - G (networkx.Graph): The knowledge graph to search.
    - node_embeddings (dict): A dictionary mapping node names to their embeddings.
    - tokenizer: Tokenizer for the embedding model.
    - model: Embedding model.
    - generate (function): A function that generates text given a prompt.
    Returns:
    - response (str): The generated reasoned response.
    - best_nodes (tuple): The best fitting nodes and their similarities.
    - path (list): The path found in the graph.
    - path_graph (networkx.Graph): The subgraph corresponding to the path.
    - shortest_path_length (int): The length of the shortest path.
    - fname (str): Filename of the HTML visualization.
    - graph_GraphML (str): Filename of the GraphML file."""
    ...
    # Build the task based on graph_analysis_type
    # 1. Task "keywords",  find all keywords in path
    ...
    # 2. Task "path and relations", find path with relations
    ...
    # 3. Task "triplets", find all triplets in path_graph
    ...
    # 4. Task "nodes and relations", find all node pairs and their edge titles in path_graph
...
def find_path_with_relations_and_reason_combined(
    ...
    """Find paths between two keywords in a graph, visualize them, and reason over them. 
    The difference to 'find_path_and_reason' is that: this function can consider multiple paths and combinations.
    Args:
         G: The knowledge graph (networkx Graph).
         node_embeddings: Embeddings for the nodes in the graph.
         tokenizer: Tokenizer for the language model.
         model: Language model for generating text.
         generate: Function to generate text given a prompt.
    Returns:
         response: The generated reasoning response."""
    ...
    # Process paths based on include_all_possible flag
